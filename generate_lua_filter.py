...
# chapnum：章採番
# numlev：採番レベル
# 図表の区切り文字、採番の区切り文字も指定したい気はするけどあんまり指定要素が多いのもな...
def generate_lua_filter(chapnum, numlev):
    lua_template = """
-- カウンタ群
local header_num = {""" + str(chapnum-1) + r"""} -- ヘッダ用
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script.py <chapnum> <numlev>")
        sys.exit(1)
    
    chapnum = int(sys.argv[1])
    numlev = int(sys.argv[2])
    
    lua_code = generate_lua_filter(chapnum, numlev)
    print(lua_code)
